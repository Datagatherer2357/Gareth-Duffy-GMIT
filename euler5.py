...
# I discerned that I had to find the lowest common multiple of these numbers. 

# After some thinking and research, I came to realise that any numbers which are divisible by 11 to 20 
# are also divisible by 1 to 10. Therefore I chose to omit integers 1 to 10 for parsimony.

# I then obtained the formulas for lowest commom multiple (lcm), and for greatest common divisor (gcd), because 
# gcd was needed as part of the lcm formula. Thus, I would need to assign these formulas as functions in Python.
...
# I created the functions "gcd", and "lcm". I scripted gcd first in order to program Python to "understand" the 
# lcm formula which would follow. 
# The lcm formula is based on finding the least commonn multiple of just two numbers, so I had to create a for loop 
# which would incorporate the range function in order to loop over all numbers from 11 to 20.
...
for i in range(11, 21): # for loop using "i" variable to iterate over all integers ranging 11 to 20.
  eul5 = lcm(eul5, i)   # argument of lcm function updated to yield smallest positive number of all integers
                        # ranging 11 to 20. Line of code (altered) was taken from: https://gist.github.com/PEZ   
