...
name = "Duffy"        # name = "Duffy" # (Assign the string to variable name)
first = name[0]       # first = name[0] # (First letter of the string name)
last = name[-1]       # last = name[-1] # (Last letter of the string name)    
firstno = ord(first)  # firstno = ord(first) # (For my surname ord(D) converts to ASCII code 68)
lastno = ord(last)    # lastno = ord(last) # (For my surname ord(y) converts to ASCII code 121)
x = firstno + lastno  # x firstno + lastno # (x (Fibonacci number) is the sum of 68 + 121
